...
// Class that takes Message objects from a queue and calls the StateMachine update.
...
int main(int argc, char** argv) {
    ...
    auto shared_message_queue = std::make_shared<util::SharedQueue<std::string>>();
    ...
    ps_client::ShowdownClientStateMachine::ContextType context{[&client](const std::string& message) {
        client.write(message);
    }};
    ps_client::ShowdownClientStateMachine state_machine(&context);
    // Add the states to the state machine.
    state_machine.AddState(ps_client::ShowdownClientStateEnum::kLoggingIn, std::make_unique<ps_client::LoginState>(ioc));
    state_machine.AddState(ps_client::ShowdownClientStateEnum::kJoinLobby, std::make_unique<ps_client::LobbyState>());
